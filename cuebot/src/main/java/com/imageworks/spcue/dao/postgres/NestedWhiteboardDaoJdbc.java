...
public class NestedWhiteboardDaoJdbc extends JdbcDaoSupport implements NestedWhiteboardDao {
    ...
    private static final Logger logger = Logger.getLogger(NestedWhiteboardDaoJdbc.class);
                ...
                if (parentGroupId != null) {
                    ...
                    if (childrenEntry == null) {
                        ...
                        logger.debug("created entry: " +
                                childrenMap.get(parentGroupId).getKey().substring(0, 4) +
                                " parent of " + groupId.substring(0, 4));
                    }
                    else {
                        logger.debug("entry existed: " + parentGroupId);
                        childrenEntry.addChild(groupId);
                    }
                }
                else {
