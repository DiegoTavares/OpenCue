    ...
    def dropEvent(self, event):
        ...
        if item and item.type() in (cuegui.Constants.TYPE_ROOTGROUP, cuegui.Constants.TYPE_GROUP):
            ...
            dialog = MoveDialog(title="Move Groups/Jobs",
                                text="Move the following into the group: %s?"%item.rpcObject.data.name,
                                event_item=item,
                                items=body_content,
                                parent=self)
            dialog.exec_()
...
class MoveDialog(QtWidgets.QDialog):
    ...
    """
    def __init__(self, title, text, event_item, items, dst_groups={}, send_to_groups=False, parent=None):
        """
